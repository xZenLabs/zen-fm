...
if "$ROOT/build.sh" --package-only "$TMP/bin" --apk "$TMP/ZenFM.apk" >"$TMP/unconfigured.log" 2>&1; then
    echo "release packaging accepted an unconfigured signing key" >&2
    exit 1
fi
grep -q 'ZENFM_RELEASE_PUBLIC_KEY_HEX' "$TMP/unconfigured.log"

"$ROOT/build.sh" --package-only "$TMP/bin" --apk "$TMP/ZenFM.apk" --dev-unsigned
...
unzip -p "$ROOT/dist/ZenFM-koreader-linux-$VERSION.zip" zenfm.koplugin/release_public_key.lua | grep -q "$PUBLIC_KEY"
